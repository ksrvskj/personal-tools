(function () {
  ...
  const EXERCISES = {
    'warm-flow': {
      ...
      dose: '4 минуты',
      rest: 'без отдыха',
      cues: ['По минуте на движение: круги кистями и плечами → «кошка-корова» на четвереньках → выпады с поворотом корпуса.', 'Темп спокойный; не тянуться через боль.'],
      mistake: 'Превращать разминку в отдельную утомительную тренировку.',
      easier: 'По 3 медленных повтора каждого движения.',
      harder: 'Усложнение не нужно: цель — разогреться.'
    },
    'split-squat': {
      name: 'Сплит-присед 1½',
      ...
      cues: ['Одна нога впереди, вторая на полшага сзади; один повтор: присед вниз → подъём до половины → снова вниз → полный подъём.', 'Вся передняя стопа прижата к полу, корпус вертикально.'],
    ...
    'archer-pushup': {
      ...
      dose: '3 × 4–8 / сторона',
      rest: '75 с после пары',
      cues: ['Упор лёжа, руки значительно шире плеч; опускаться к одной ладони — вторая рука остаётся прямой.', 'Корпус и таз — одна линия; грудь идёт к рабочей ладони.'],
    ...
    'hamstring-walkout': {
      ...
      dose: '3 × 6–10 выходов',
      rest: '60 с',
      cues: ['Лёжа на спине поднять таз (ягодичный мост) и мелкими шагами уводить пятки вперёд до почти прямых ног, затем так же вернуть — это один повтор.', 'Таз всё время держится высоко.'],
      mistake: 'Проваливать таз или шагать слишком далеко сразу.',
      easier: 'Ягодичный мост с пятками дальше от таза, 3 × 10.',
      harder: 'В дальней точке задержаться на 2 секунды, удерживая таз высоким.',
    ...
    'calf-raise': {
      ...
      rest: '45 с',
      cues: ['Стоя на одной ноге, рука слегка на стене для равновесия: подъём на носок и медленное опускание 2–3 секунды.', 'Подниматься через большой палец, не наружный край стопы.'],
    ...
    'rkc-plank': {
      ...
      seconds: 20,
      rest: '45 с',
      cues: ['Планка на предплечьях; изо всех сил тянуть локти к стопам, а стопы к локтям — тело не двигается, но напряжено целиком.', 'Ровно выдыхать; поясницу не прогибать, таз не поднимать.'],
    ...
    'side-plank-leg': {
      ...
      seconds: 20,
      rest: '30–45 с',
      cues: ['Лёжа на боку с опорой на предплечье (локоть под плечом), поднять таз — тело прямой линией; верхнюю ногу поднять примерно на 30 см и держать.', 'Таз слегка подан вперёд; верхняя пятка тянется вдаль, а не вверх.'],
    ...
    'reverse-crunch': {
      ...
      dose: '2 × 8–12',
      rest: '45 с',
      cues: ['Лёжа на спине, ноги подняты и согнуты; подкрутить таз к рёбрам — колени идут к груди.', 'Опускать крестец медленно, без маха ногами.'],
    ...
    'pogo': {
      ...
      seconds: 10,
      rest: '40–50 с',
      cues: ['Прыжки на месте на передней части стопы: низко, тихо и упруго, колени мягкие.', 'Перед началом — 1–2 минуты марша и подъёмов на носки.'],
    ...
    'skater-squat': {
      ...
      dose: '3 × 5–8 / сторона',
      rest: '75 с после пары',
      cues: ['Стоя на одной ноге, вторая согнута сзади; садиться назад-вниз, пока заднее колено мягко не коснётся сложенного коврика.', 'Большой палец опорной стопы не отрывается от пола.'],
    ...
    'pike-pushup': {
      ...
      dose: '3 × 5–10',
      rest: '75 с после пары',
      cues: ['Из упора лёжа поднять таз — тело углом; сгибая локти, вести макушку к полу между ладонями, не перед ними.', 'Локти направлены назад по диагонали.'],
    ...
    'single-leg-rdl': {
      ...
      dose: '3 × 8–12 / сторона',
      rest: '60 с',
      cues: ['Стоя на одной ноге: наклон корпуса вперёд с прямой спиной, свободная нога уходит назад — макушка и пятка тянутся в разные стороны.', 'Таз не разворачивать: обе тазовые кости смотрят в пол.'],
    ...
    'soleus-raise': {
      ...
      rest: '45 с',
      cues: ['Подъём на носок с согнутым коленом — колено остаётся согнутым всё повторение; можно придерживаться за стену.', 'Подъём высокий, опускание контролируемое.'],
      mistake: 'Заваливать стопу на внешний край.',
    ...
    'bear-tap': {
      ...
      dose: '3 × 6–10 / сторона',
      rest: '45 с',
      cues: ['На четвереньках оторвать колени на 2–3 см от пола; из этого положения касаться ладонью противоположного плеча.', 'Таз не двигается, дыхание ровное.'],
      mistake: 'Переносить вес на опорную руку и перекашивать таз.',
    ...
    'side-plank-dip': {
      ...
      dose: '2 × 8–12 / сторона',
      rest: '30–45 с',
      cues: ['Боковая планка на предплечье; опускать таз почти до пола и поднимать обратно.', 'Плечо над локтем; двигается только таз.'],
    ...
    'dead-bug': {
      ...
      dose: '2 × 6–10 / сторона',
      rest: '30–45 с',
      cues: ['Лёжа на спине, руки вверх, колени согнуты под 90°; медленно опускать противоположные руку и ногу к полу и возвращать.', 'Поясница прижата к коврику; полный выдох при выпрямлении.'],
    ...
    'side-plank-reach': {
      ...
      dose: '3 × 6–10 / сторона',
      rest: '30–45 с',
      cues: ['Боковая планка; верхняя рука тянется к потолку, затем с поворотом груди проносится в «окно» под корпусом и обратно.', 'Таз всё время высоко.'],
    ...
    'hollow-hold': {
      ...
      seconds: 20,
      rest: '45 с',
      cues: ['Лёжа на спине: поясница прижата, лопатки и прямые ноги подняты над полом — тело «лодочкой».', 'Высота ног — та, при которой поясница не отрывается.'],
      mistake: 'Прижимать подбородок к груди и задерживать дыхание.',
    ...
    'reverse-plank': {
      ...
      seconds: 25,
      rest: '45 с',
      cues: ['Сидя, ладони на полу за тазом; поднять таз — тело прямой линией лицом вверх.', 'Толкать пол ладонями, раскрыть грудь, сжать ягодицы.'],
    ...
    'reverse-lunge': {
      ...
      dose: '2 × 8 / сторона',
      rest: '30 с',
      cues: ['Шаг назад в выпад; вставая, толкнуться всей передней стопой и вынести заднее колено вверх — секундная пауза на одной ноге.', 'Корпус вертикально, без заваливания вперёд.'],
    ...
    'pushup': {
      ...
      dose: '2 × 6–12',
      rest: '45 с',
      cues: ['Отжимания в выбранном варианте; заканчивать подход, пока в запасе остаются 1–3 чистых повтора.', 'Тело — одна линия: грудь и таз поднимаются одновременно.'],
    ...
    'active-hang': {
      ...
      seconds: 15,
      rest: '60 с',
      cues: ['Вис на перекладине на прямых руках; плечи мягко опущены от ушей, а не поджаты к ним.', 'Без прогиба в пояснице и раскачивания.'],
    ...
    'scap-pull': {
      ...
      dose: '4 × 5–8',
      rest: '60–75 с',
      cues: ['Вис на прямых руках; не сгибая локти, опустить плечи и чуть приподнять грудь — двигаются только лопатки.', 'Движение контролируемое, без раскачивания корпуса.'],
    ...
    'assisted-pullup': {
      ...
      dose: '4 × 4–6',
      rest: '90 с',
      cues: ['Подтягивание с носками на опоре (стул, ступень): ноги помогают ровно настолько, насколько нужно.', 'Начинать движение лопатками; опускаться 3 секунды.'],
    ...
    'negative-pullup': {
      ...
      seconds: 6,
      rest: '90–120 с',
      cues: ['С низкой перекладины или устойчивой ступени без прыжка занять верхнее положение (подбородок над перекладиной) и медленно опуститься на прямые руки.', 'Опускаться равномерно; закончить подход, если следующий спуск уже нельзя контролировать хотя бы 3 секунды.'],
    ...
    'strict-pullup': {
      ...
      rest: '2–3 минуты',
      cues: ['Из активного виса подтянуться без раскачки, пока подбородок не окажется над перекладиной, и опуститься на прямые руки.', 'Остановиться, пока следующий повтор ещё получается чисто.'],
  ...
  function renderBar() {
    ...
    return '<div class="wo-plan-view">' +
      ...
      (!hasBar
        ? '<section class="wo-banner"><strong>Сегодня без турника.</strong><p>Ступени можно смотреть и без турника; текущая сохраняется.</p>' +
